...
const TABS = [
  ...
  { href: '/admin/reports', key: 'reports', label: '신고 관리' },
] as const;

export default function AdminHeader({ active, right }: { active: 'stats' | 'members' | 'reports'; right?: ReactNode }) {
  ...
  return (
    <div style={{ height: 54, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 26px', background: '#fff', borderBottom: '1px solid #ebebeb' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <Link href="/admin/stats" style={{ textDecoration: 'none' }}><TangbisilLogo size={20} /></Link>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button
          ...
          style={{ padding: '6px 14px', border: '1px solid #dadce0', borderRadius: 8, fontSize: 13, color: '#5f6368', background: '#fff', cursor: 'pointer' }}
        >
          로그아웃
        </button>
      </div>
